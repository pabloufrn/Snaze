...
{
    // por causa da implemetação, não temos acesso a janela atual
    
    sui::Button* bt1 = new sui::Button("Start Game", (sf::Vector2f) {0, 200}, 30, std::bind(&MainMenuState::start_game, this));
    sui::Button* bt2 = new sui::Button("Help", (sf::Vector2f) {0, bt1->get_position().y + bt1->get_size().y + 30}, 30);
    sui::Button* bt3 = new sui::Button("About", (sf::Vector2f) {0, bt2->get_position().y + bt2->get_size().y + 30}, 30);
    sui::Button* bt4 = new sui::Button("Exit", (sf::Vector2f) {0, bt3->get_position().y + bt3->get_size().y + 30}, 30, std::bind(&MainMenuState::exit_game, this));
